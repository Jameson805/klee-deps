...
pin_root=""
benchmarks_csv=""
default_benchmarks=(mbedtls mbedtls_sliced libgcrypt libgcrypt_sliced openssl openssl_sliced openssl_almeida bearssl)
default_benchmarks+=(appliedcryp ghostrider libg pycrypto)
selected_benchmarks=("${default_benchmarks[@]}")

usage() {
    cat <<EOF
Usage: $0 [--sym-size <n>] [--loop-max-iterations <n>] [--max-solver-time <duration>] [--kill-after <duration>] [--max-memory <n>] [--mod-exp-only] [--search <strategies>] [--concretize-on-solver-timeout <bool>] [--solver-backend <stp|metasmt|dummy|z3>] [--optimize-array <false|all|index|value>] [--benchmarks <list>] <max_time>
  ...
  --benchmarks <list>      - optional, comma-separated benchmark groups to run
        valid: mbedtls,mbedtls_sliced,libgcrypt,libgcrypt_sliced,openssl,openssl_sliced,openssl_almeida,bearssl,appliedcryp,ghostrider,libg,pycrypto
...
if [[ -n "$benchmarks_csv" ]]; then
    ...
    for raw in "${requested_benchmarks[@]}"; do
        ...
        [[ -z "$bench" ]] && continue
        case "$bench" in
            mbedtls|mbedtls_sliced|libgcrypt|libgcrypt_sliced|openssl|openssl_sliced|openssl_almeida|bearssl|appliedcryp|ghostrider|libg|pycrypto)
...
results_dir="$repo_root/results/klee_cf_results"
rm -rf "$results_dir"
mkdir -p "$results_dir"
...
run_openssl_sliced() {
    ...
    run_openssl_algo_sliced() {
        ...
        run_case "OpenSSL 1.1.1q (Sliced) ${algo} (Fix Pub)" "benchmarks/openssl-1.1.1q/klee_fix_pub_${algo}.bc" "openssl_${algo}_fix_pub_sliced" "benchmarks/openssl-1.1.1q/klee_fix_pub_replay_${algo}" "--secret exp" "ctchecker_results/openSSL_1_1_1q/${algo}-3.json" "openssl-1.1.1q" "$memory_flag" --ctchecker-prefix "crypto/bn"
        run_case "OpenSSL 1.1.1q (Sliced) ${algo} (Var Pub)" "benchmarks/openssl-1.1.1q/klee_var_pub_${algo}.bc" "openssl_${algo}_var_pub_sliced" "benchmarks/openssl-1.1.1q/klee_var_pub_replay_${algo}" "--secret exp --public base,mod" "ctchecker_results/openSSL_1_1_1q/${algo}-3.json" "openssl-1.1.1q" "$memory_flag" --ctchecker-prefix "crypto/bn"
        run_case "OpenSSL 1.1.1q (Sliced) ${algo} (Var Pub Lim Loop Break)" "benchmarks/openssl-1.1.1q/klee_var_pub_lim_loop_break_${algo}.bc" "openssl_${algo}_var_pub_lim_loop_break_sliced" "benchmarks/openssl-1.1.1q/klee_var_pub_replay_${algo}" "--secret exp --public base,mod" "ctchecker_results/openSSL_1_1_1q/${algo}-3.json" "openssl-1.1.1q" "$memory_flag" --ctchecker-prefix "crypto/bn"
    }
...
run_openssl_almeida() {
    ...
    benchmarks/openssl_almeida/build.sh --klee-cf --preset default
...
run_appliedcryp() {
    ...
    benchmarks/appliedCryp/build.sh --klee-cf --preset default
...
run_ghostrider() {
    ...
    benchmarks/ghostrider/build.sh --klee-cf --preset default
...
run_libg() {
    ...
    benchmarks/libg/build.sh --klee-cf --preset default
...
run_pycrypto() {
    ...
    benchmarks/pycrypto/build.sh --klee-cf --preset default
