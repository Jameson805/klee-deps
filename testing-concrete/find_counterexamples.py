...
def parse_metadata(c_file_path):
    ...
    def parse_var_declaration(line):
        vars_out = []
        tokens = [v.strip() for v in line.split(":")[1].split(",")]
        for tok in tokens:
            array_match = re.match(r"(\w+)\[(\d+)\]", tok)
            if array_match:
                name, count = array_match.groups()
                for i in range(int(count)):
                    vars_out.append(f"{name}[{i}]")
            else:
                vars_out.append(tok)
        return vars_out
    ...
    with open(c_file_path, 'r') as f:
        for line in f:
            ...
            if line.startswith("// VARS:"):
                variables = parse_var_declaration(line)
            elif line.startswith("// PUBLIC:"):
                public_vars = parse_var_declaration(line)
    ...
    for line in lines:
        ...
        if "klee_make_symbolic" in line:
            for var in public_vars:
                if var in inputs and var in line:
                    val = inputs[var]
                    new_lines.append(f"    klee_assume({var} == {val});\n")
