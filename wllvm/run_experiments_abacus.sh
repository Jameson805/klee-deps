...
num_copies=10
temp_dir="/tmp"
output_base="./abacus_experiments"
abacus_root=""
sym_sizes=(4 16)
...
Options:
  ...
  --num-copies N         Number of parallel workspace copies (default: 2)
...
Notes:
	...
	- It parallelizes by making temporary copies of the current workspace and running
		wllvm/run_abacus.sh in each copy using parallel_klee_copies.sh.
	- Collected output subdir is wllvm/abacus_results under each worker destination.
...
run_postprocess() {
	...
	for sym in "${sym_sizes[@]}"; do
		...
		run_tagged "$tag" "$merge_json_runner" "$dst" || return 1
	done
}
...
for sym in "${sym_sizes[@]}"; do
	...
	run_tagged "ABACUS SYM ${sym}" \
		...
		"wllvm/abacus_results" "$dst" -- \
		wllvm/run_abacus.sh "$abacus_root" --sym-size "$sym"
done
