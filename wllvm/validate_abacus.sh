...
timeout=300
sym_size_override=""
reproduce_script="./reproduce_positives.py"
...
Options:
  --results-dir DIR        Input directory of raw Abacus JSON files (default: wllvm/abacus_results)
  ...
  --reproduce-script PATH  Path to reproduce_positives.py
  -h, --help               Show this help

Notes:
  - This script is intended to run on the host, outside the Abacus container.
  - It validates the JSON produced by wllvm/run_abacus.sh using host-side klee_fix_pub_replay executables.
EOF
}
...
resolve_replay_executable() {
    ...
    case "$json_name" in
        mbedtls.json)
            printf '%s\n' 'mbedtls-3.2.1/klee_fix_pub_replay'
            ;;
        libgcrypt.json)
            printf '%s\n' 'libgcrypt-and-libgpg-error/klee_fix_pub_replay'
            ;;
        openssl_recp.json)
            printf '%s\n' 'openssl-1.1.1q/klee_fix_pub_replay_recp'
            ;;
        openssl_mont.json)
            printf '%s\n' 'openssl-1.1.1q/klee_fix_pub_replay_mont'
            ;;
        openssl_mont_consttime.json)
            printf '%s\n' 'openssl-1.1.1q/klee_fix_pub_replay_mont_consttime'
            ;;
        openssl_mont_word.json)
            printf '%s\n' 'openssl-1.1.1q/klee_fix_pub_replay_mont_word'
            ;;
        *)
            return 1
            ;;
